...
function renderStoryboard() {
    ...
    state.storyboards.forEach((sb, index) => {
        ...
        row.innerHTML = `
            <td class="sticky-col-1 px-2 py-2 text-center"><input type="checkbox" class="cursor-pointer row-checkbox" data-mirror-id="${sb.mirror_id}"></td>
            <td class="sticky-col-2 px-2 py-2 font-mono text-xs text-slate-400">${sb.sequence_number}</td>
            <td class="sticky-col-3 px-2 py-2 font-bold text-sm text-primary">${sb.mirror_id}</td>
...
function renderCharacters() {
    const tbody = document.getElementById('character-tbody');
    if (!tbody) return;
    ...
    if (state.characters.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-slate-400">暂无角色数据</td>
            </tr>
        `;
        return;
    }
    ...
    state.characters.forEach((char, index) => {
        const row = document.createElement('tr');
        row.className = `hover-row transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`;
        row.innerHTML = `
            <td class="px-2 py-2 font-bold text-sm text-primary">${char.name}</td>
            <td class="px-2 py-2 text-xs">${char.description || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-500">${char.image_prompt_zh || '-'}</td>
            <td class="px-2 py-2 text-xs font-mono leading-tight">${char.image_prompt_en || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-400">${char.notes || '-'}</td>
        `;
...
function renderScenes() {
    ...
    if (state.scenes.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-slate-400">暂无场景数据</td>
            </tr>
        `;
        return;
    }
    ...
    state.scenes.forEach((scene, index) => {
        ...
        row.className = `hover-row transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`;
        row.innerHTML = `
            <td class="px-2 py-2 font-bold text-sm text-primary">${scene.name}</td>
            <td class="px-2 py-2 text-xs">${scene.description || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-500">${scene.image_prompt_zh || '-'}</td>
            <td class="px-2 py-2 text-xs font-mono leading-tight">${scene.image_prompt_en || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-400">${scene.notes || '-'}</td>
        `;
        tbody.appendChild(row);
    });
}
...
function renderProps() {
    ...
    if (state.props.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-slate-400">暂无道具数据</td>
            </tr>
        `;
        return;
    }
    ...
    state.props.forEach((prop, index) => {
        ...
        row.className = `hover-row transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`;
        row.innerHTML = `
            <td class="px-2 py-2 font-bold text-sm text-primary">${prop.name}</td>
            <td class="px-2 py-2 text-xs">${prop.description || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-500">${prop.image_prompt_zh || '-'}</td>
            <td class="px-2 py-2 text-xs font-mono leading-tight">${prop.image_prompt_en || '-'}</td>
            <td class="px-2 py-2 text-xs text-slate-400">${prop.notes || '-'}</td>
        `;
        tbody.appendChild(row);
    });
}
...
async function sendChatMessage() {
    ...
    try {
        ...
        console.log('分镜数量:', structuredData?.storyboards?.length);
        console.log('角色数量:', structuredData?.characters?.length);
        console.log('场景数量:', structuredData?.scenes?.length);
        console.log('道具数量:', structuredData?.props?.length);

        if (structuredData) {
            // 为每个分镜添加自动生成的 sequence_number（从 1 开始）
            const storyboardsWithSequence = (structuredData.storyboards || []).map((sb, index) => ({
                ...sb,
                sequence_number: index + 1
            }));

            console.log('添加序号后的分镜:', storyboardsWithSequence);

            // 保存结构化数据到数据库
            await invoke('save_generated_data', {
                folderPath: state.currentProject,
                storyboards: storyboardsWithSequence,
